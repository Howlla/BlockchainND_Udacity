...
// TODO Create an event to emit when a solution is added
	event solutionAdded(address solverAddress);



// TODO Create a function to add the solutions to the array and emit the event
    function addSolution(address _solverAddress, uint256 _id, uint[2] memory A, uint[2] memory A_p, uint[2][2] memory B, uint[2] memory B_p, uint[2] memory C, uint[2] memory C_p, uint[2] memory H, uint[2] memory K, uint[2] memory input) public {   
        ...
        Solution memory sol = Solution({id: _id, solverAddress: _solverAddress});
        solutionsSubmitted[key] = sol;
        solutions.push(sol);
        emit solutionAdded(_solverAddress);
    }

    function isSolutionUnique(bytes32 key) public view returns (bool){
    	bool isUnique = true;
    	if (solutionsSubmitted[key].solverAddress != address(0)){
    		isUnique = false;
    	}
		return isUnique;	
    }

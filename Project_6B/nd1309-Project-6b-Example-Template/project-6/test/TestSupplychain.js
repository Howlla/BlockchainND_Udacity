...
contract('SupplyChain', function(accounts) {
    ...
    const originManufacturerName = "Bhavye Sharma";
    const originManufacturerInformation = "Delhi";
    const originManufacturerLatitude = "53.1034089";
    const originManufacturerLongitude = "57.6295649";
    var productID = sku + upc
    const productNotes = "Marlboro Advance Box"
    ...
    // 1st Test
    it("Testing smart contract function ProcessItem() that allows a Manufacturer to process item", async() => {
        ...
        // const resultBufferTwo = await supplyChain.fetchItemData.call(upc)

        // Verify the result set
        assert.equal(resultBufferOne[0], sku, 'Error: Invalid item SKU');
        assert.equal(resultBufferOne[1], upc, 'Error: Invalid item UPC');
        assert.equal(resultBufferOne[2], originManufacturerID, 'Error: Missing or Invalid ownerID');
        assert.equal(resultBufferOne[3], originManufacturerID, 'Error: Missing or Invalid ManufacturerID');
        assert.equal(resultBufferOne[4], originManufacturerName, 'Error: Missing or Invalid ManufacturerName');
        assert.equal(resultBufferOne[5], itemState, 'Error: Invalid item State');
        assert.equal(eventEmitted, true, 'Invalid event emitted');
    })    

    // 2nd Test
    it("Testing smart contract function packItem() that allows a farmer to pack the item", async() => {
        const supplyChain = await SupplyChain.deployed()
    ...
    // 7th Test
    it("Testing smart contract function fetchItem() that allows anyone to fetch data about an item", async() => {
        const supplyChain = await SupplyChain.deployed()
        ...
        assert.equal(resultBufferOne[3], originManufacturerID, 'Error: Missing or Invalid ManufacturerID');
        assert.equal(resultBufferOne[4], originManufacturerName, 'Error: Missing or Invalid ManufacturerName');
        assert.equal(resultBufferOne[5], 5, 'Error: Invalid item State');
        assert.equal(resultBufferOne[6], productPrice, 'Error: Invalid item price');
        assert.equal(resultBufferOne[7], retailerID, 'Error: Missing or Invalid retailer ID');
        assert.equal(resultBufferOne[8], customerID, 'Error: Missing or Invalid customer ID');

        // Verify the result set
        assert.equal(resultBufferTwo[0], productID, 'Error: Invalid item productID');
        assert.equal(resultBufferTwo[1], upc, 'Error: Invalid item UPC');
        assert.equal(resultBufferTwo[2], productPrice, 'Error: Missing or Invalid productPrice');
        assert.equal(resultBufferTwo[3], productNotes, 'Error: Missing or Invalid productNotes');
        assert.equal(resultBufferTwo[4], originManufacturerName, 'Error: Missing or Invalid originManufacturerName');
        assert.equal(resultBufferTwo[5], originManufacturerInformation, 'Error: Missing or Invalid originManufacturerInformation');
        assert.equal(resultBufferTwo[6], originManufacturerLatitude, 'Error: Missing or Invalid originManufacturerLatitude');
        assert.equal(resultBufferTwo[7], originManufacturerLongitude, 'Error: Missing or Invalid originManufacturerLongitude');
    })    
